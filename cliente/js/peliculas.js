...
$(document).ready(function () {
  ...
  $.getJSON(servidor + "/generos", function (data) {
    for (let i = 0; i < data.generos.length; i++) {
      ...
      //a esa opcion se le asigna como valor el id del genero, dato que luego va a servir
      //para filtrar por id de genero.
      opcion.attr("value", data.generos[i].id_genero);
...
function ControladorPeliculas() {
  ...
  (this.buscarPeliculas = function (pagina, cantidad) {
    ...
    $.getJSON(servidor + "/peliculas?" + query, function (data) {
